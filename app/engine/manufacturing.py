...
Industry job cost formula:
  job_cost = sum(adjusted_price[material] * qty) * system_cost_index * (1 + facility_tax)
"""
...
def calc_material_quantity(
    ...
    """
    Returns total material quantity needed across all runs.

    structure_me_bonus: fractional reduction, e.g. 0.02 for 2%.
    """
...
def calc_time_seconds(
    ...
    """Returns total time in seconds for the job."""
...
async def analyse_blueprint(
    ...
    """
    Analyse a single blueprint and return a ManufacturingOption with
    material requirements checked against current inventory.
    Returns None if the blueprint has no manufacturing activity in SDE.
    """
    ...
    # Fetch required materials from SDE
    ...
    # Fetch market prices for cost estimation
    price_result = await db.execute(
        select(MarketPrice.type_id, MarketPrice.adjusted_price)
    )
    prices = {row.type_id: row.adjusted_price or 0.0 for row in price_result.fetchall()}
    ...
    for mat in sde_materials:
        ...
        total_material_cost += prices.get(mat.material_type_id, 0.0) * needed
...
async def get_all_manufacturing_options(
    ...
    """
    Analyse all blueprints across given characters.
    Returns list sorted: buildable-now first, then by product name.
    """
    ...
    options.sort(key=lambda o: (not o.can_build_now, o.product_name))
    return options
